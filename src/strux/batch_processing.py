...
class BatchProcessor(Generic[T]):
    ...
    def _validate_outputs(
        ...
        for field_name, field_config in config.field_configs.items():
            ...
            if is_first_run:
                ...
                if field_config.compare_with_annotation and annotations:
                    expected_values = [
                        getattr(annotation, field_name) if annotation else None for annotation in annotations
                    ]
                else:
        ...
        return [
            StepValidation(
                step_name="batch_validation", field_validations=validations, metadata={"is_first_run": is_first_run}
            )
        ]

    def process_batch(
        self, batch_df: pd.DataFrame, steps: list[Step], config: RegressionConfig[T], is_first_run: bool = False
    ) -> BatchResults:
        ...
        for step in steps:
            if hasattr(step.inference_fn, "batch_process"):
                ...
                outputs = batch_df.apply(lambda row: step.run(row.to_dict()), axis=1).to_list()

            # Store raw outputs for later diff computation
            raw_outputs[step.name] = outputs

            # Validate outputs using the instance method
            validations = self.validate_outputs(outputs, config, is_first_run)
