...
class HTMLReport:
    ...
    def _prepare_data(self, results: "RegressionResults") -> Dict[str, Any]:
        ...
        for validation in all_validations:
            ...
            field_data["plots"][validation.field_name] = plot.to_html(full_html=False, include_plotlyjs=False)
        ...
        data = {
            ...
            "fields": fields,
            "inputs": [
                input_model.model_dump() if hasattr(input_model, "model_dump") else input_model
                for input_model in all_inputs
            ],
            "outputs": [
                output_model.model_dump() if hasattr(output_model, "model_dump") else output_model
    ...
    def _generate_plots(self, data: Dict[str, Any]) -> Dict[str, str]:
        ...
        for field_name, field_data in data.get("fields", {}).items():
            ...
            if is_numeric:
                ...
                fig.add_trace(
                    go.Scatter(
                        x=indices, y=baseline_values, name="Baseline", mode="lines+markers", line=dict(color="blue")
                    )
                )

                fig.add_trace(
                    go.Scatter(
                        x=indices, y=current_values, name="Current", mode="lines+markers", line=dict(color="green")
                    )
                )

                # Add status indicators
                for i, sample in enumerate(samples):
                    color = {"improved": "green", "unchanged": "gray", "regressed": "red"}.get(sample["status"], "gray")
                ...
                fig.add_trace(go.Bar(x=categories, y=baseline_counts, name="Baseline", marker_color="blue"))

                fig.add_trace(go.Bar(x=categories, y=current_counts, name="Current", marker_color="green"))
            ...
            # Convert to HTML
            plots[field_name] = fig.to_html(full_html=False, include_plotlyjs="cdn", config={"displayModeBar": False})
    ...
    def _prepare_comparison_data(self, results: "RegressionResults") -> Dict[str, Any]:
        ...
        comparison_data = {
            ...
            "fields": {},
            "changes": {"improvements": 0, "regressions": 0, "unchanged": 0},
        }
        ...
        annotations = []
        if (
            results.config.annotation_field
            and results.config.data_source
            and hasattr(results.config.data_source, "data")
        ):
        ...
        for step_validation in diff_validations:
            for field_validation in step_validation.field_validations:
                ...
                field_data = {
                    ...
                    "samples": [],
                    "metrics": {"improvements": 0, "regressions": 0, "unchanged": 0},
                }
    ...
    def generate(
        self, results: "RegressionResults", comparison: Optional["RegressionResults"] = None, include_plots: bool = True
    ) -> str:
        ...
        if results.baseline_results:
            ...
            template = self.env.get_template(
                "annotation_report.html" if results.is_annotation_based else "experiment_report.html"
            )
