...
def create_annotation_plot(field_data: Dict[str, Any]) -> go.Figure:
    ...
    if isinstance(predictions[0], str):
        ...
        if annotations:
            ...
            fig.add_annotation(
                text=f"Accuracy: {accuracy:.2%}", xref="paper", yref="paper", x=0.5, y=-0.15, showarrow=False
            )
    else:
        # For numeric data (like ratings)
        fig.add_trace(
            go.Scatter(x=list(range(len(predictions))), y=predictions, name="Predictions", mode="lines+markers")
        )

        if annotations:
            fig.add_trace(
                go.Scatter(x=list(range(len(annotations))), y=annotations, name="Ground Truth", mode="lines+markers")
            )

            # Add error distribution
            errors = [abs(p - a) for p, a in zip(predictions, annotations)]
            fig.add_trace(go.Box(y=errors, name="Error Distribution", yaxis="y2"))
            ...
            fig.add_annotation(
                text=f"MAE: {mae:.2f}<br>RMSE: {rmse:.2f}", xref="paper", yref="paper", x=1.15, y=0.5, showarrow=False
            )
...
def create_experiment_plot(field_data: Dict[str, Any]) -> go.Figure:
    ...
    if isinstance(current_values[0], str):
        ...
        fig.add_trace(go.Bar(x=labels, y=[current_dist[label] for label in labels], name="Current Run"))

        if baseline_values:
            baseline_dist = {label: baseline_values.count(label) for label in labels}
            fig.add_trace(go.Bar(x=labels, y=[baseline_dist[label] for label in labels], name="Baseline"))
        ...
        # For numeric data
        fig.add_trace(
            go.Scatter(x=list(range(len(current_values))), y=current_values, name="Current Run", mode="lines+markers")
        )

        if baseline_values:
            fig.add_trace(
                go.Scatter(
                    x=list(range(len(baseline_values))), y=baseline_values, name="Baseline", mode="lines+markers"
                )
            )

            # Add difference distribution
            diffs = [c - b for c, b in zip(current_values, baseline_values)]
            fig.add_trace(go.Box(y=diffs, name="Differences", yaxis="y2"))
    ...
    fig.update_layout(
        ...
        showlegend=True,
        yaxis2=dict(title="Differences", overlaying="y", side="right"),
        barmode="group",
    )
